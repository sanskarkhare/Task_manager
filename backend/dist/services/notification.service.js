"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NotificationService = void 0;
const notification_repository_1 = require("../repositories/notification.repository");
const socketHandler_1 = require("../socket/socketHandler");
class NotificationService {
    constructor() {
        this.notificationRepo = new notification_repository_1.NotificationRepository();
    }
    createNotification(userId, message, taskId) {
        return __awaiter(this, void 0, void 0, function* () {
            const notification = yield this.notificationRepo.create({ userId, message, taskId });
            // Emit real-time notification
            try {
                (0, socketHandler_1.getIO)().to(userId).emit('notification', notification);
            }
            catch (e) {
                console.error('Socket notification emit failed', e);
            }
            return notification;
        });
    }
    getUserNotifications(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.notificationRepo.findByUserId(userId);
        });
    }
    markAsRead(id) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.notificationRepo.markAsRead(id);
        });
    }
}
exports.NotificationService = NotificationService;
